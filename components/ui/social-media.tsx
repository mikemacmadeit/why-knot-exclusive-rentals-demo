...
/** A single social media link with Lucide icon. */
...
export interface SocialTooltipProps extends React.HTMLAttributes<HTMLUListElement> {
  items: SocialItem[];
  /** Tooltip placement relative to the icon. Default: "top" (footer-friendly). */
  tooltipSide?: "top" | "bottom";
}

const SocialTooltip = React.forwardRef<HTMLUListElement, SocialTooltipProps>(
  ({ className, items, tooltipSide = "top", ...props }, ref) => {
    ...
    const baseTooltipStyles = cn(
      "absolute left-1/2 -translate-x-1/2 px-2.5 py-1.5 text-sm text-white whitespace-nowrap rounded-md opacity-0 invisible transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:visible",
      tooltipSide === "top"
        ? "bottom-full mb-2 group-hover:mb-3"
        : "top-full mt-2 group-hover:mt-3"
    );
          ...
          return (
            <li key={item.ariaLabel} className="relative group">
              ...
              >
                ...
                />
                <Icon className={cn(baseSvgStyles)} aria-hidden />
              </a>
              <div
